...
def index(request):
    if request.method == 'GET':
        ...
        capacite = 50
        context = {'role': role, 'segment': 'index', "capacite": capacite, "patients": get_patients(),
                   "nb_staff": len(get_staffs()),
...
def doctors(request):
    ...
    return render(request, 'home/doctors.html', {"role": role, "segment": "doctors", "doctors": doctors})
...
def patients(request):
    ...
    return render(request, 'home/patients.html', {"role": role, "segment": "patients", "patients": data})
...
def staffs(request):
    ...
    return render(request, 'home/staffs.html', {"role": role, "segment": "staffs", "staffs": data})
...
def addAlert(request):
    ...
    if (request.method == "POST"):
        ...
        alert = create_alert(_doctor=None, _createdTime=createdTime, _responseTime=None, _response=None, _user=user,
                             _content=content, _priority=priority)
        return redirect(index)
...
def respondAlert(request):
    ...
    if (request.method == "POST"):
        ...
        alert = edit_alert(_id=_id, _doctor=doctor, _createdTime=createdTime, _responseTime=responseTime,
                           _response=response, _user=user, _content=content, _priority=priority)
        return redirect(index)
...
def addPatient(request):
    ...
    if (request.method == "POST"):
        ...
        profile_pic = request.POST.get('profile_pic', 'avatar.png')
...
def editPatient(request):
    ...
    if (request.method == "POST"):
        ...
        mobile = request.POST.get('mobile')
        address = request.POST.get('address')
        profile_pic = request.POST.get('profile_pic', patient.user.profile_pic)
        status = request.POST.get('status')
...
def addDoctor(request):
    ...
    if (request.method == "POST"):
        ...
        doctor = create_doctor(_nom=nom, _prenom=prenom, _email=email, _mobile=mobile, _address=address,
                               _profilepic=profile_pic,
...
def editStaff(request):
    ...
    if (request.method == "POST"):
        ...
        mobile = request.POST.get('mobile')
        address = request.POST.get('address')
        profile_pic = request.POST.get('profile_pic', staff.user.profile_pic)
        status = request.POST.get('status')
        ...
        for id in [i.id for i in get_rooms()]:
            if (request.POST.get(id) != None):
                rooms.append(request.POST.get(id))
        department = request.POST.get('department')
        staff = edit_staff(_id=_id, _nom=nom, _prenom=prenom, _mobile=mobile, _address=address, _profilepic=profile_pic,
...
def editDoctor(request):
    ...
    if (request.method == "POST"):
        ...
        mobile = request.POST.get('mobile')
        address = request.POST.get('address')
        profile_pic = request.POST.get('profile_pic', doctor.user.profile_pic)
        status = request.POST.get('status')
...
def rooms(request):
    ...
    return render(request, 'home/rooms.html', {'role': role, "segment": "rooms", "rooms": data})
...
def profile(request):
    ...
    role = currentuser.role
    if role == "doctor":
        doctor = [i for i in get_doctors() if i.user.id == currentuser.id][0]
        rooms = get_roomsByIDs(doctor.rooms)
        patients = []
        for room in rooms:
            patients += get_patientsByIDs(room.patients)
        ...
        staff = [i for i in get_staffs() if i.user.id == currentuser.id][0]
        rooms = get_roomsByIDs(staff.rooms)
        patients = []
        for room in rooms:
            patients += get_patientsByIDs(room.patients)
        ...
        patient = [i for i in get_patients() if i.user.id == currentuser.id][0]
        room = None
        doctor = None
        if patient.room != None and patient.room != "":
...
def addRoom(request):
    ...
    if (request.method == "POST"):
        ...
        room = create_room(_number=number, _doctor=doctor, _department=department, _staff=staff, _patients=[],
                           _nbBeds=nbbeds)
...
def editRoom(request):
    if (request.method == "GET"):
        ...
        if room.doctor != None and room.doctor != "":
            doctor = get_doctor(room.doctor)
        return render(request, 'home/editRoom.html',
                      {"room": room, "currentdoctor": doctor, "doctors": doctors, "list_checked": list_checked,
    ...
    if (request.method == "POST"):
        ...
        room = edit_room(_id=_id, _number=number, _doctor=doctor, _department=department, _staff=staff,
                         _patients=room.patients, _nbBeds=nbbeds)
        return redirect(rooms)
